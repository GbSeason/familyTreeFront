<template>
  <div>
    <div class="labels">
      <div>
        {{ parseInt(targetScale * 100) }}%
      </div>
    </div>

    <div v-bind="{ scopedSlots: $scopedSlots }" class="orgchart-container" @wheel="zoom && zoomHandler($event)"
      @mouseup="pan && panning && panEndHandler($event)" @mousedown="pan && panStartHandler($event)"
      @mousemove="pan && panning && panHandler($event)" :style="{ cursor: cursorVal }">
      <div class="orgchart" :style="{ transform: transformVal, cursor: cursorVal }">
  ...
  methods: {
    ...
    setChartScale(newScale) {
      ...
      if (this.transformVal === '') {
        ...
        if (this.transformVal.indexOf('3d') === -1) {
          this.targetScale = Math.abs(window.parseFloat(matrix[3]) * newScale).toFixed(1)
          if (this.targetScale > this.zoomoutLimit && this.targetScale < this.zoominLimit) {
            matrix[0] = 'matrix(' + this.targetScale
            matrix[3] = this.targetScale
            this.transformVal = matrix.join(',')
          }
        } else {
          this.targetScale = Math.abs(window.parseFloat(matrix[5]) * newScale).toFixed(1)
          if (this.targetScale > this.zoomoutLimit && this.targetScale < this.zoominLimit) {
            matrix[0] = 'matrix3d(' + this.targetScale
            matrix[5] = this.targetScale
            this.transformVal = matrix.join(',')
          }
        }
      }
    ...
    zoomHandler(e) {
      let newScale = 1 + (e.deltaY > 0 ? -0.1 : 0.1)
...
.orgchart-container {
  ...
  /* background-image: linear-gradient(90deg,
      rgba(200, 0, 0, 0.05) 10%,
      rgba(0, 0, 0, 0) 10%),
    linear-gradient(rgba(200, 0, 0, 0.05) 10%, rgba(0, 0, 0, 0) 10%);
...
.orgchart .lines .topLine {
  border-top: 2px solid #26b9ff;
}

.orgchart .lines .rightLine {
  border-right: 1px solid #26b9ff;
  float: none;
  border-radius: 0;
}

.orgchart .lines .leftLine {
  border-left: 1px solid #26b9ff;
  float: none;
  border-radius: 0;
}
